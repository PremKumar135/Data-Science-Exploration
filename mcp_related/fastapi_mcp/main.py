...
FAMOUS_TRAINERS = {
    "ash": ["pikachu", "charizard", "squirtle", "bulbasaur", "greninja", 
...
async def get_pokemon_details(name_or_id):
    ...
    async with httpx.AsyncClient() as client:
        ...
        return PokemonDetail(id=pokemon_data["id"],
...
async def compare_pokemon(pokemon_names: List[str] = Query(..., min_length=2, max_length=6)):
    ...
    for stat in ["hp", "attack", "defense", "special-attack", "special-defense", "speed"]:
        comparison["stats"][stat] = {"highest": max([(p.name, p.stats.get(stat, 0)) for p in pokemon_list], key=lambda x: x[1]),
...
async def get_team(user_id: str = Path(..., description="User ID to get the team for")):
    ...
    return TeamResponse(user_id=user_id,
...
async def remove_from_team(user_id: str = Path(..., description="User ID to remove Pokemon from"),
